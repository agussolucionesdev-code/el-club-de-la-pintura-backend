...
async function ensureExpenseSchema(): Promise<void> {
  ...
  const ddl: string[] = [
    ...
    `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name='Expense_supplierId_fkey' AND table_name='Expense') THEN ALTER TABLE "Expense" ADD CONSTRAINT "Expense_supplierId_fkey" FOREIGN KEY ("supplierId") REFERENCES "Supplier"("id") ON DELETE SET NULL ON UPDATE CASCADE; END IF; END $$`,
    ...
    `CREATE TABLE IF NOT EXISTS "ExpenseBudget" ("id" SERIAL NOT NULL, "branchId" INTEGER, "category" TEXT, "amount" DECIMAL(14,2) NOT NULL, "period" TEXT NOT NULL DEFAULT 'MONTHLY', "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP, "updatedAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT "ExpenseBudget_pkey" PRIMARY KEY ("id"))`,
    `CREATE UNIQUE INDEX IF NOT EXISTS "ExpenseBudget_branchId_category_key" ON "ExpenseBudget"("branchId", "category")`,
    `CREATE TABLE IF NOT EXISTS "RecurringExpense" ("id" SERIAL NOT NULL, "amount" DECIMAL(14,2) NOT NULL, "reason" TEXT NOT NULL, "category" TEXT NOT NULL, "type" TEXT NOT NULL DEFAULT 'VARIABLE', "frequency" TEXT NOT NULL DEFAULT 'MONTHLY', "branchId" INTEGER NOT NULL, "supplierId" INTEGER, "active" BOOLEAN NOT NULL DEFAULT true, "createdById" INTEGER NOT NULL, "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP, "updatedAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT "RecurringExpense_pkey" PRIMARY KEY ("id"))`,
  ...
  for (const sql of ddl) {
    try {
      ...
      logger.error("[STARTUP] ensureExpenseSchema failed:", err);
    }
  }
  logger.info("[STARTUP] ensureExpenseSchema: expense tables and columns verified.");
}
