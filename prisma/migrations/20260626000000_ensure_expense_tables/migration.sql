-- Idempotent: ensures ExpenseBudget and RecurringExpense tables exist.
-- Safe to run even if previous migration applied partially.

-- ── Expense: new columns (idempotent) ──
...
-- FK (only add if not already there — safe: IF NOT EXISTS guard via DO block)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'Expense_supplierId_fkey'
